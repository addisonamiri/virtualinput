/*virtualinput aggiornato 5/01/2010 no t9*/ 
#include <gtk/gtk.h>
#include <X11/Xlib.h>
...
/* Crea una FrameT9 */
 ...
 gtk_scrolled_window_add_with_viewport( GTK_SCROLLED_WINDOW(scrolled_window),str->tp.gtklist);//nuovo x lista
 gtk_container_add(GTK_CONTAINER(framet9), str->tp.gtklist);
                ...
                else if(retval == SQLITE_ROW)
		{
			// SQLITE_ROW means fetched a row
		    	...
		    	for(int col=0 ; col<cols;col++)
		   	{
				...
				{
					printf ("%s",val);
					sprintf(vetparole[str->tp.numparoletrovate-1].parola,"%s",val);
				...
				{
					printf ("\tfr=%s",val);
					vetparole[str->tp.numparoletrovate-1].frequenza=atoi(val);
				}
		   	 }  
		}
		else
...
void sigh_print_selection(GtkWidget *gtklist, struttura *str)
{
 ...
 if (vetparole[str->tp.indice].frequenza>1) 
	...
	sprintf (query, "insert into personale (codice,parola,frequenza) values (\'%s\',\'%s\',1);",str->tp.codicet9,vetparole[str->tp.indice].parola);

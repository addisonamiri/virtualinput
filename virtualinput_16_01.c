/*virtualinput aggiornato 16/01/2011*/ 
#include <gtk/gtk.h>
#include <X11/Xlib.h>
...
funz listF_Buttons[NUM_OF_BUTTONS-10]={fButtonShift,fButtonAltGR,fButtonDelete,fButtonSpace,fButtonEnter,fButtonTP,fButtonTab,fButtonIns,movetoleft,movetoright,dockicon};
...
void movetoleft(struttura *str)
{printf("muivimento a sinistra\n");
 if(str->position==2)
 {
	gtk_window_set_gravity(GTK_WINDOW(str->window),GDK_GRAVITY_SOUTH_WEST);
 	gtk_window_move(GTK_WINDOW(str->window),0,str->s_height-WINDOW_HEIGHT);
	str->position=1;
...
void finestra(struttura *str)
{
 ...
 int i, x, y;
 char s_i[3];
 char s[NUM_OF_BUTTONS][10]={"1","2 abc","3 def","4 ghi","5 jkl","6 mno","7 pqrs","8 tuv","9 wxyz","⇧ shift","0 +","alt gr","←","space","↵","txt pr","tab","ins","←","→","."};

/*virtualinput aggiornato 20/01/2011*/ 
#include <gtk/gtk.h>
#include <gdk/gdk.h>
#include <X11/Xlib.h>
...
void button_pressed_destroy (GtkDialog *dialog,struttura *str)
{
  if (str->ins.on) 
   g_print("cancel\n");//ciò che avviene al cancel va messo qui
}
...
void button_pressed_insert(GtkWidget *widget,struttura *str)
{
  ...
  entry_text = gtk_entry_get_text (GTK_ENTRY (str->ins.entry));
...
{ 
  ...
  str->ins.entry = gtk_entry_new ();
  gtk_entry_set_max_length (GTK_ENTRY (str->ins.entry), LEN_WORD_DB);
  gtk_entry_set_text (GTK_ENTRY (str->ins.entry), "");
  gtk_box_pack_start (GTK_BOX(gtk_dialog_get_content_area(dialog)), str->ins.entry, TRUE, TRUE, 0);
  
  gtk_widget_show (str->ins.entry);
  ...
  g_signal_connect (dialog, "destroy", G_CALLBACK (button_pressed_destroy), str); 

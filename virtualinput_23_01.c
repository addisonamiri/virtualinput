/*virtualinput aggiornato 23/01/2011*/ 
#include <gtk/gtk.h>
#include <gdk/gdk.h>
#include <X11/Xlib.h>
...
/*Funzione associata al tasto invio */
{
 if(str->ins.on==0) 
 ...
 else
  {
   ...
   //str->listButton[0].status=XK_ISO_Level5_Shift;
  // str->listButton[0].len=LENB0;
                                   /*quando passa in modalità inserimento per evitare casino con le query premendo 0 deve 
                                   stampare solo il carttere '0', quindi quando disabilitiamo la modalità inserimento
                                   dobbiamo ristabilire le condizioni normali */ 
   str->ins.index++;
...
// str->listButton[0].len=1;
// str->listButton[0].status=0;
                           /*quando passa in modalità inserimento per evitare casino con le query premendo 0 deve 
                             stampare  solo il carttere '0'*/

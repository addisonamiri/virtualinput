/*virtualinput aggiornato 21/01/2011*/ 
#include <gtk/gtk.h>
#include <gdk/gdk.h>
#include <X11/Xlib.h>
...
typedef struct tinsert{ /*la struttura contiene i dati che servono neel caso di un nuovo inserimento nel dizionario*/
 ...
 GtkWidget *winDialog;//window dialog
}tins;
 ...
 if (str->tp.statot9==0)
  {
   ...
   str->tp.codicet9[0]=0;
   gtk_label_set_text (GTK_LABEL (str->mylabel2),"PT activated");
  }
 else
  { 
   str->tp.statot9=0;
   gtk_label_set_text (GTK_LABEL (str->mylabel2),"PT disableb");
  }
}
...
void updatedictionary(struttura *str)
{
       ...
       char query[200];
       if(str->ins.index==-1)
	 return;//inserimento annullato ripassiamo da modalità insert a modalità t9 
	
	//Check if the word is present on the global dictionary
	sprintf (query, " select * from  globale where parola like \'%s\';",str->ins.new_word);
	//printf("\n%s\n",query);
	...
	if(retval != SQLITE_ROW)
	{
		...
		sprintf (query, "insert into personale (codice,parola,frequenza) values (\'%s\',\'%s\',0);",str->ins.key_numbers,str->ins.new_word);
...
void ins(struttura *str)
{
 ...
 str->ins.on=1;
 str->ins.index=-1;
 bzero(str->ins.key_numbers,LEN_WORD_DB+1);
 bzero(str->ins.new_word,LEN_WORD_DB+1);
 str->modifier=0;
 gtk_label_set_text (GTK_LABEL (str->mylabel),"");
 str->tp.statot9=0;//disabilitiamo il t9
 gtk_label_set_text (GTK_LABEL (str->mylabel2),"ins new word,↵");
}
...
{ 
 ...
 if(button<=9){
   if ((str->tp.statot9==0)||((str->ins.on)&&(str->ins.index<LEN_WORD_DB)))
...
void finestra(struttura *str)
{
 ...
 char s[NUM_OF_BUTTONS][10]={"1","2 abc","3 def","4 ghi","5 jkl","6 mno","7 pqrs","8 tuv","9 wxyz","⇧ shift","0","alt gr","del","space","↵","txt pr","tab",".","←","↓","→"};
...
*/
 ...
 str->mylabel2 = gtk_label_new ("PT disabled");
 ...
 gtk_widget_set_usize(scrolled_window, 0, 30);
 gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolled_window),GTK_POLICY_AUTOMATIC, GTK_POLICY_NEVER);
 gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(scrolled_window),str->tp.gtklist);
...
void button_pressed_cancel(GtkWidget *widget, struttura *str)
{
  gtk_widget_destroy(str->ins.winDialog);
}
...
{ 
  ...
  GtkDialog *dialog;
  char slabel[LEN_WORD_DB+15];
  str->ins.winDialog = gtk_dialog_new();
  gtk_widget_get_toplevel(str->window);
  dialog=GTK_DIALOG(str->ins.winDialog);
  gtk_window_set_title(GTK_WINDOW(str->ins.winDialog), "new word?");
...
*/
  ...
  g_signal_connect(G_OBJECT(button1), "clicked", G_CALLBACK(button_pressed_cancel), str);
...
int main(int argc, char *argv[])
{
 ...
 int keycodemap[LENB*7+LENB1+LENB5+LENB10]={XK_1,XK_backslash,XK_comma,XK_plus,XK_minus,XK_less,XK_quoteright,XK_period,
                                          ...
                                          XK_0/*0*/,XK_space/* */,XK_question/*?*/,XK_exclam/*!*/,XK_equal/*=*/,XK_asterisk/* * */,XK_semicolon/*;*/,XK_colon/*:*/,XK_underscore/*_*/,XK_2/*"*/,XK_3/*£*/,XK_dollar/*$*/,XK_percent/*%*/,XK_ampersand/*&*/,XK_slash/*/*/,XK_bar/*|*/,XK_parenleft/*(*/,XK_parenright/*)*/,XK_asciicircum/*^*/,XK_greater/*>*/};
 ...
 for(i++,sup+=LENB5;i<=9;i++,sup+=LENB)
  str.listButton[i].simbol=setListButton(LENB,keycodemap+sup,str.listButton[i],0);
 str.listButton[0].simbol=setListButton(LENB10,keycodemap+sup,str.listButton[0],XK_ISO_Level5_Shift);

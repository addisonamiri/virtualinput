/*virtualinput aggiornato 02/02/2011*/ 
#include <gtk/gtk.h>
#include <gdk/gdk.h>
#include <X11/Xlib.h>
...
#define PATH_IMG "keyboard_icon.gif"
...
typedef struct tinsert{ /*la struttura contiene i dati che servono neel caso di un nuovo inserimento nel dizionario*/
 ...
 unsigned char new_word[LEN_WORD_DB+1];
}tins;
...
/*Funzione associata al tasto 'AltGr' */
{
  if (str->modifier!=XK_Control_R)
   printModifier(str,XK_Control_R,"Alt Gr on");
  else
   printModifier(str,0,"");
}
...
void movetoright(struttura *str)
{
 str->move.position_width=(str->move.position_width==str->move.delay_orizzontale*2)?0:str->move.position_width+str->move.delay_orizzontale;
 setMove(str);
}
...
void finestra(struttura *str)
{
 ...
 char s[NUM_OF_BUTTONS][10]={"1","2 abc","3 def","4 ghi","5 jkl","6 mno","7 pqrs","8 tuv","9 wxyz","⇧ shift","0 +","alt gr","del","space","↵","PT","tab","ins","→","↓","."};
...
void gestionet9 (struttura *str)
{
	...
	sprintf (query, " select parola,codice,frequenza from globale where codice like \'%s%%\' order by codice,frequenza  limit 0,%d;",str->tp.codicet9,N);
  	//GtkWidget *list_item;
	GList *dlist=NULL;
	gchar *s;
 ...
 for (int kk=0; kk < dim_parola; kk++) 
 {
	...
	*let=0;//equals sprintf(let,"");
	...
	sprintf(let,"%c",word[kk]);
        ...
        sendtofocus(str,(int)str->tp.vetparole[str->tp.indice].parola[kk]);
}
// printf("\n");
...
{ 
  ...
  snprintf(slabel,LEN_WORD_DB+16,"insert '%s' in db?",str->ins.new_word);
...
/*setta i modificatori per tutti i caratteri ascii*/
{
 int i,modifiers_temp[N_SIMBOL_MODIFIER]={
            ...
            0,XK_ISO_Level5_Shift,XK_ISO_Level5_Shift,XK_Control_R,XK_ISO_Level5_Shift,/* !"#$*/

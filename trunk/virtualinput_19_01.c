/*virtualinput aggiornato 19/01/2011*/ 
#include <gtk/gtk.h>
#include <gdk/gdk.h>
#include <X11/Xlib.h>
...
typedef struct tinsert{ /*la struttura contiene i dati che servono neel caso di un nuovo inserimento nel dizionario*/
 ...
 int index;//indice 
 char key_numbers[30];//numeri associati per il t9 alla nuova parola
}tins;
...
{ 
 ...
 if((button!=9)&&(button<11)){//&!T9
   if ((str->tp.statot9==0)||((str->ins.on)&&(str->ins.index<29))){
     XGetInputFocus(str->display, &str->win, &revert);
     writeSimbol(str,&str->listButton[button],button+1);
    }
   else 
    {
...
void window_insert(struttura *str)
{
    ...
    button = gtk_button_new_with_label("ins");//gtk_button_new_from_stock (GTK_STOCK_CLOSE);

/*virtualinput aggiornato 4/01/2010 no t9*/ 
#include <gtk/gtk.h>
#include <X11/Xlib.h>
...
{
 str->tp.statot9=(str->tp.statot9==0)?1:0;
}
void fButtonIns(struttura *str)
...
void finestra_origine(struttura *str)
{
 ...
 gtk_window_set_default_size (GTK_WINDOW(window), NULL, NULL);

 /* Crea una Frame */
 frame = gtk_frame_new(NULL);
 gtk_container_add(GTK_CONTAINER(window), frame);
 /* Setta l'etichetta della frame */
 gtk_frame_set_label( GTK_FRAME(frame), "virtualinput" );
...
{
 GtkWidget *window, *frame, *framet9, *winbox, *box, *tbox, *table, *scrolled_window;//nuovo x lista

 GtkWidget **buttonArray;

 int i, x, y;
 char s_i[3];
 char s[18][10]={"1","2 abc","3 def","4 ghi","5 jkl","6 mno","7 pqrs","8 tuv","9 wxyz","⇧ shift","0 +","alt gr","←","space","↵","txt pr","tab","ins"};
 gtk_init (NULL,NULL);
 window = gtk_window_new (GTK_WINDOW_POPUP);
...
/* Crea una FrameT9 */
 ...
 gtk_container_add(GTK_CONTAINER(tbox), scrolled_window);
 gtk_widget_show(scrolled_window);

 //inserimento lista
 gtklist=gtk_list_new();
 gtk_list_set_selection_mode((GtkList *) gtklist, GTK_SELECTION_SINGLE);
 gtk_scrolled_window_add_with_viewport( GTK_SCROLLED_WINDOW(scrolled_window),gtklist);//nuovo x lista
 gtk_container_add(GTK_CONTAINER(framet9), gtklist);
 gtk_widget_show(gtklist);

 gtk_signal_connect(GTK_OBJECT(gtklist),"selection_changed",GTK_SIGNAL_FUNC(sigh_print_selection),str);//nuovo x lista
...
{
 GtkWidget *window, *framet9, *winbox, *tbox,*scrolled_window;
 gtk_init (NULL,NULL);
			...
			// SQLITE_ROW means fetched a row
		    	...
		    	for(int col=0 ; col<cols;col++)
		   	{
				...
				if (col==0) 
				{
					...
					gtk_object_set_data(GTK_OBJECT(list_item), list_item_data_key,s);
...
void sigh_print_selection(GtkWidget *gtklist, struttura *str)
{
 ...
 g_print("Selection 2\n");
 int dim_parola = strlen(vetparole[str->tp.indice].parola);
  g_print("Selection 3\n");
 ...
 for (int kk=0; kk < dim_parola; kk++) 
 {
	...
	sprintf(word,"%s",vetparole[str->tp.indice].parola);
 ...
 vetparole[str->tp.indice].frequenza=vetparole[str->tp.indice].frequenza+1;
        printf("\nNuova frequenza parola selezionata: %d\n",vetparole[str->tp.indice].frequenza);

 char query[200];
 bzero (query,200);
 if (vetparole[str->tp.indice].frequenza>1) 
	sprintf (query, "update personale set frequenza =%d where parola =\'%s\';",vetparole[str->tp.indice].frequenza,vetparole[str->tp.indice].parola);
 else 
	sprintf (query, "insert into personale (codice,parola,frequenza) values (\'%s\',\'%s\',1);",codicet9,vetparole[str->tp.indice].parola);

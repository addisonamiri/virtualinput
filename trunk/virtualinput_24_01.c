...
funz listF_Buttons[NUM_OF_BUTTONS-10]={fButtonShift,fButtonAltGR,fButtonDelete,fButtonSpace,fButtonEnter,fButtonTP,fButtonTab,fButtonIns,movetoup,movetoright,dockicon};
...
/*Funzione associata al tasto TP */
{
 if (str->tp.statot9==0)
  ...
  { 
   ins(str);
   str->tp.statot9=0;
   gtk_list_clear_items ((GtkList *) str->tp.gtklist,0,N);//svuolatamo la lista
...
void finestra(struttura *str)
{
 ...
 char s[NUM_OF_BUTTONS][10]={"1","2 abc","3 def","4 ghi","5 jkl","6 mno","7 pqrs","8 tuv","9 wxyz","⇧ shift","0","alt gr","del","space","↵","txt pr","tab","ins","↓","→","."};
	...
	*s=0;//equals sprintf(s,"");
	...
	const char *val = (const char*)sqlite3_column_text(str->tp.stmt,col);
			...
			//		printf ("%s",val);
					...
					dlist=g_list_append(dlist,(char *) val);
					gtk_widget_show(str->tp.list_item);
					//g_object_set_data(G_OBJECT(str->tp.list_item), list_item_data_key,(void *)val);

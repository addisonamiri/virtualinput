/*virtualinput aggiornato 03/02/2011*/ 
#include <gtk/gtk.h>
#include <gdk/gdk.h>
#include <X11/Xlib.h>
...
void updatedictionary(struttura *str)
{
	...
	sprintf (query, " select parola from  globale where parola like \'%s\';",str->ins.new_word);
	//printf("\n%s\n",query);
	...
	if(retval != SQLITE_ROW)
	{
		...
		sprintf (query, "insert into globale (codice,parola,frequenza) values (\'%s\',\'%s\',3);",str->ins.key_numbers,str->ins.new_word);
